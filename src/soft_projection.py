...
def knn_point(group_size, point_cloud, query_cloud):
    ...
    dist, idx = knn_obj(point_cloud.contiguous(), query_cloud.contiguous())
    return dist, idx
...
class SoftProjection(nn.Module):
    def __init__(
        ...
        self.return_grouping = return_grouping
    ...
    def forward(self, point_cloud, query_cloud, point_features=None, action="project"):
        ...
        if action == "project":
            ...
            return self.project_and_propagate(
                point_cloud, point_features, query_cloud, self.return_grouping
            )
        else:
            raise ValueError(
    ...
    def project_and_propagate(
        self, point_cloud, point_features, query_cloud, return_grouping=False
    ):
        ...
        if not return_grouping:
            return (projected_points, propagated_features, None, None)
        else:
            return (
                projected_points,
                propagated_features,
    ...
    def project(self, point_cloud, query_cloud, hard=False):
        ...
        return self.project_group(grouped_points, query_cloud, hard=hard)

    def project_group(self, grouped_points, query_cloud, hard=False):
        dist = self._get_distances(grouped_points, query_cloud)
